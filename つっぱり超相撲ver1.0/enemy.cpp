			...
			if (pULTGauge->GetUlt(0) == true && m_bUltDis == false)
			{
				if (m_pAnimation == NULL)
				{
					m_pAnimation = CBAnimation::Create(D3DXVECTOR3(pos), D3DXCOLOR(1.0f, 1.0f, 0.0f, 1.0f),
						100.0f, 150.0f, 0.0625f, 1.0f, 1.5f, 16, 0, 0, 1, CLoad::TEXTURE_ANIMATION);
				}
				m_bUltDis = true;
			}
...
// つっぱりとの当たり判定
//============================================================================
bool CEnemy::TsuppariCollision(D3DXVECTOR3 pos)
{
	// プレイヤー取得
	CPlayer *pPlayer = NULL;
	CSansoGauge *pSansoGauge = NULL;
	...
	{
		bHit = pPlayer->GetTuppari().Collision(&pos, &D3DXVECTOR3(m_posOld.x, m_posOld.y + 1.0f, m_posOld.z), &m_move, TSUPPARI_COLLISION);
		//つっぱりにあたった
		if (bHit == true)
		{
			//衝撃波
			CBAnimation::Create(D3DXVECTOR3(posEnemy.x, posEnemy.y + 10.0f, posEnemy.z - 30.0f), D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f),
